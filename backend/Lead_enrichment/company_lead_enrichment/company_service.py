...
Implements all 6 Company Enrichment Flow priorities:
  ...
  P3  Company AI Analysis
      ...
      Uses WB_LLM_HOST → HuggingFace fallback → rule-based fallback
...
async def init_company_db() -> None:
    ...
    async with get_pool().acquire() as conn:
        ...
        for col, col_type in [
            ...
            ("crm_brief",           "TEXT"),   # Qwen2.5-7B full CRM profile (JSON)
...
async def _fetch_bd_company_profile(company_linkedin_url: str) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=120) as client:
            ...
            if snap_id:
                from Lead_enrichment.bulk_lead_enrichment.lead_enrichment_brightdata_service import poll_snapshot
...
async def _call_llm(messages: list[dict], max_tokens: int = 600) -> Optional[str]:
    """Shared LLM caller — WB_LLM_HOST → HuggingFace → None."""
