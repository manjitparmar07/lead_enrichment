...
─────────────────────────
HuggingFace-powered AI enrichment modules.
...
MODEL_SMART = "Qwen/Qwen2.5-72B-Instruct"
...
def _hf_model() -> str:
    try:
        import keys_service as ks
        return ks.get("HF_MODEL") or os.getenv("HF_MODEL", MODEL_SMART)
    except Exception:
        return os.getenv("HF_MODEL", MODEL_SMART)
...
async def _hf(
    ...
    """
    Call HuggingFace Router API → (parsed_result_dict, token_usage_dict).
    Falls back to MODEL_FAST + truncated prompt on failure.
    """
    token = _hf_token()
    if not token:
        raise RuntimeError("HF_TOKEN not configured")

    hf_model = _hf_model() if model == MODEL_SMART else model

    attempts = [(hf_model, user), (MODEL_FAST, user[:3000])]
    ...
    for attempt_model, attempt_user in attempts:
        ...
        if r.is_success:
            break
        last_err = f"HF {r.status_code}: {r.text[:300]}"
        logger.warning("[HF] Error on %s — %s", attempt_model, last_err)

    if not r or not r.is_success:
        raise RuntimeError(f"HuggingFace failed: {last_err}")
...
async def run_identity(
    ...
    result, tu = await _hf(sys, usr, model or MODEL_FAST, _temp(temperature, 0.2))
...
async def run_pitch_intelligence(
    ...
    result, tu = await _hf(sys, usr, model or MODEL_SMART, _temp(temperature, 0.2))
    return _add_usage(result, tu)
...
async def run_outreach(
    ...
    result, tu = await _hf(sys, usr, model or MODEL_SMART, _temp(temperature, 0.3))
    return _add_usage(result, tu)
