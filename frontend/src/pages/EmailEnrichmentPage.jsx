...
const API = `${import.meta.env.VITE_BACKEND_URL || 'https://leadenrichment-production-5b78.up.railway.app'}/api`
const POLL_MS = 3000
...
export default function EmailEnrichmentPage() {
  ...
  // ── Email Validator state ───────────────────────────────────────────────────
  const [validatorEmails, setValidatorEmails] = useState('')
  const [validating, setValidating] = useState(false)
  const [validateResults, setValidateResults] = useState(null)  // {total, valid, invalid, results}
  const [validateLeadsRunning, setValidateLeadsRunning] = useState(false)
  const [validateLeadsJob, setValidateLeadsJob] = useState(null)
  ...
  // ── Validate emails (manual list) ─────────────────────────────────────────
  const validateEmails = async () => {
    const emails = validatorEmails.split(/[\n,;]+/).map(e => e.trim()).filter(Boolean)
    if (!emails.length) { toast.error('Enter at least one email address'); return }
    if (emails.length > 100) { toast.error('Maximum 100 emails per batch'); return }
    setValidating(true)
    setValidateResults(null)
    try {
      const res = await fetch(`${API}/email-enrich/validate`, {
        method: 'POST',
        headers: authHeaders(),
      ...
      toast.success(`Validated ${data.total} emails — ${data.valid} valid, ${data.invalid} invalid`)
    } catch (e) {
  ...
  // ── Validate all lead emails (background job) ──────────────────────────────
  const pollValidateLeadsJob = useCallback(async (jobId) => {
    try {
      const res = await fetch(`${API}/email-enrich/jobs/${jobId}`, { headers: authHeaders() })
      if (!res.ok) return
      const data = await res.json()
      setValidateLeadsJob(data)
      if (data.status === 'completed' || data.status === 'failed') {
        clearInterval(validatePollRef.current)
        setValidateLeadsRunning(false)
        if (data.status === 'completed') {
          toast.success(`Validation done — ${data.valid} valid, ${data.invalid} invalid out of ${data.total}`)
        } else {
  ...
  useEffect(() => () => clearInterval(validatePollRef.current), [])

  const startValidateLeads = async () => {
    setValidateLeadsRunning(true)
    setValidateLeadsJob(null)
    try {
      const res = await fetch(`${API}/email-enrich/validate-leads`, {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ limit: 5000, save_results: validateSave }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.detail || JSON.stringify(data))
      toast.success(`Validation job started — ${data.total} leads queued`)
      setValidateLeadsJob({ job_id: data.job_id, status: 'running', total: data.total, processed: 0, valid: 0, invalid: 0 })
      validatePollRef.current = setInterval(() => pollValidateLeadsJob(data.job_id), POLL_MS)
    } catch (e) {
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ ...card, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)' }}>Email Validator</div>
            <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2 }}>
              Paste email addresses (one per line, max 100) to check validity via ValidEmail.net
            </div>
          </div>
        </div>
        ...
        <textarea
          ...
          style={{
            ...
            color: 'var(--text-1)', fontFamily: 'monospace', resize: 'vertical',
            outline: 'none', marginBottom: 12,
          }}
        />

        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <button style={btn('primary', validating)} disabled={validating} onClick={validateEmails}>
            {validating ? <><Spinner /> Validating…</> : (
              <>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.85 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.96 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/>
                </svg>
                Validate Emails
              </>
            )}
          ...
          {validateResults && (
            <button style={btn('ghost')} onClick={() => { setValidateResults(null); setValidatorEmails('') }}>
              Clear
            </button>
          )}
          {validatorEmails && (
            <span style={{ fontSize: 11, color: 'var(--text-3)' }}>
              {validatorEmails.split(/[\n,;]+/).filter(e => e.trim()).length} email(s) entered
            </span>
          )}
        </div>
        ...
        {validateResults && (
          <div style={{ marginTop: 16 }}>
            <div style={{ display: 'flex', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
              <span style={tag('blue')}>Total: {validateResults.total}</span>
              <span style={tag('green')}>Valid: {validateResults.valid}</span>
              <span style={tag('red')}>Invalid: {validateResults.invalid}</span>
            </div>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border-1)' }}>
                    {['Email', 'Valid', 'Score', 'Disposable', 'Role', 'DNS', 'SMTP', 'Reason'].map(h => (
                      <th key={h} style={{ padding: '6px 10px', textAlign: 'left', color: 'var(--text-3)', fontWeight: 600, whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {validateResults.results.map((r, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid var(--border-1)' }}>
                      <td style={{ padding: '8px 10px', fontFamily: 'monospace', fontSize: 11, color: 'var(--text-1)' }}>{r.email}</td>
                      <td style={{ padding: '8px 10px' }}>
                        <span style={tag(r.valid ? 'green' : 'red')}>{r.valid ? 'VALID' : 'INVALID'}</span>
                      </td>
                      <td style={{ padding: '8px 10px', color: r.score >= 0.7 ? '#6ee7b7' : r.score >= 0.4 ? '#fcd34d' : '#fca5a5', fontWeight: 600 }}>
                        {r.score != null ? (r.score * 100).toFixed(0) + '%' : '—'}
                      </td>
                      <td style={{ padding: '8px 10px' }}>
                        <span style={tag(r.disposable ? 'amber' : 'gray')}>{r.disposable ? 'YES' : 'NO'}</span>
                      </td>
                      <td style={{ padding: '8px 10px' }}>
                        <span style={tag(r.role ? 'amber' : 'gray')}>{r.role ? 'ROLE' : '—'}</span>
                      </td>
                      <td style={{ padding: '8px 10px' }}>
                        <span style={tag(r.dns_valid ? 'green' : 'red')}>{r.dns_valid ? '✓' : '✗'}</span>
                      </td>
                      <td style={{ padding: '8px 10px' }}>
                        <span style={tag(r.smtp_valid ? 'green' : r.smtp_valid === false ? 'red' : 'gray')}>
                          {r.smtp_valid === true ? '✓' : r.smtp_valid === false ? '✗' : '?'}
                        </span>
                      </td>
                      <td style={{ padding: '8px 10px', color: 'var(--text-3)', fontSize: 11 }}>{r.reason || r.error || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div style={{ ...card, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-3)' }}>
              Runs ValidEmail.net against every enriched lead email in your org and optionally saves the bounce risk score back to the database.
            </div>
            <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 10, cursor: 'pointer', fontSize: 12, color: 'var(--text-2)' }}>
              <input
                type="checkbox"
                checked={validateSave}
                onChange={e => setValidateSave(e.target.checked)}
                disabled={validateLeadsRunning}
                style={{ accentColor: '#6366f1' }}
              />
              Save bounce risk to database
            </label>
          </div>
          <button
            style={btn('primary', validateLeadsRunning)}
            disabled={validateLeadsRunning}
            onClick={startValidateLeads}
          >
            {validateLeadsRunning ? <><Spinner /> Running…</> : (
              <>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        {validateLeadsJob && (
          <div style={{ marginTop: 16, padding: 14, background: 'var(--bg-base)', borderRadius: 10, border: '1px solid var(--border-1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 12 }}>
              <span style={{ color: 'var(--text-2)', fontWeight: 600 }}>
                {validateLeadsJob.status === 'completed' ? 'Completed' : 'Validating…'}
              </span>
              <span style={{ color: 'var(--text-3)' }}>
                {(validateLeadsJob.processed || 0).toLocaleString()} / {(validateLeadsJob.total || 0).toLocaleString()}
                {validateLeadsJob.valid != null && ` · ${validateLeadsJob.valid} valid · ${validateLeadsJob.invalid} invalid`}
              </span>
            </div>
            <ProgressBar pct={validateLeadsJob.total > 0 ? Math.round(((validateLeadsJob.processed || 0) / validateLeadsJob.total) * 100) : 0} />
            <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
              <span style={tag(validateLeadsJob.status === 'completed' ? 'green' : validateLeadsJob.status === 'failed' ? 'red' : 'blue')}>
                {validateLeadsJob.status?.toUpperCase()}
              </span>
              {validateLeadsJob.status === 'completed' && validateSave && (
                <span style={{ fontSize: 11, color: 'var(--text-3)' }}>Bounce risk saved to DB</span>
              )}
            </div>
          </div>
        )}
