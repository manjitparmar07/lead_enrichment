...
──────────────────────────
CRUD for custom API features + execution via HuggingFace / WorksBuddy LLM.
"""
...
async def create_feature(org_id: str, data: dict) -> dict:
    async with get_pool().acquire() as conn:
        row = await conn.fetchrow(
            ...
            data.get("model_provider", "huggingface"),
            data.get("model_name"),
...
async def execute_feature(
    ...
    provider   = (feature.get("model_provider") or "huggingface").lower()

    if provider == "worksbuddy":
