...
def _hf_model() -> str:
    try:
        import keys_service as ks
        return ks.get("HF_MODEL") or os.getenv("HF_MODEL", "Qwen/Qwen2.5-7B-Instruct")
    except Exception:
...
async def _hf(
    ...
    """
    Call HuggingFace Router → (parsed_result_dict, token_usage_dict).
    Uses Qwen2.5-7B-Instruct by default (HF_MODEL env var).
    """
    token = _hf_token()
    if not token:
        raise RuntimeError("HF_TOKEN not configured")

    mdl = model or _hf_model()
    payload = {
        "model": mdl,
        "messages": [
    ...
    if r.status_code == 503:
        raise RuntimeError(f"HuggingFace model '{mdl}' is loading — retry shortly")
    if not r.is_success:
        raise RuntimeError(f"HuggingFace {r.status_code}: {r.text[:400]}")
