...
# ═════════════════════════════════════════════════════════════════════════════
# AI Worker System — raw_profile → LLM (Qwen 70B / Groq / WB) → completed
#
# Architecture:
...
async def _run_single_ai(
    ...
    Process one lead through the LLM pipeline:
      ...
      3. Call LLM (HF Qwen → Groq → WB, controlled by semaphore)
    ...
    try:
        ...
        async with _get_hf_semaphore():
            response_text = await _call_llm(
                ...
                max_tokens=2000,
                temperature=0.2,
                hf_first=True,   # Qwen 70B → Groq fallback → WB LLM
            )
